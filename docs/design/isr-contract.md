...
Laravel (ドメインイベント)
  ...
  → ジョブが Next.js の再検証エンドポイントを HTTP で叩く
      POST http://web:3000/internal/revalidate   ← East-West(ローカル: compose のサービス名 / 本番: ECS Service Connect)
...
- **パスは `/internal/revalidate`**(`/api/*` にしない・2026-07-06 追記): `/api/*` は Next.js の rewrites / CloudFront で Laravel へ振られるパス空間であり衝突する。また `/internal/*` は公開経路(CloudFront/ALB のルーティング対象)に露出させず、シークレットとパス設計の二重で外部からの失効叩きを防ぐ。
